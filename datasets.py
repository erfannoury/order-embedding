...
def load_dataset(name, cnn, load_train=True, fold=0):
    ...
    for split in splits:
        ...
        dataset[split]['ims'] = numpy.load(
            '%s/images/%s/%s.npy' % (loc, cnn, splitName))

        # handle coco specially by only taking 1k or 5k captions/images
        if name == 'coco' and split in ['dev', 'test']:
            dataset[split]['ims'] = dataset[split][
                'ims'][fold * 1000:(fold + 1) * 1000]
            dataset[split]['caps'] = dataset[split][
                'caps'][fold * 5000:(fold + 1) * 5000]
